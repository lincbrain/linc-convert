...
from linc_convert.utils.io.zarr import ZarrPythonArray, from_config
...
def get_zarr_array(inp: str, dandiset_id: Union[str, None]) -> "ZarrPythonGroup":
    if dandiset_id is None:
        return ZarrPythonGroup.open(inp)["0"]
    return ZarrPythonGroup.open_dandi(dandiset_id=dandiset_id, asset_path=inp)
...
def convert_spool_zarr(
    ...
    *,
    ...
    """
    using_zarr = inp.endswith(".ome.zarr")
    ...
    all_tiles_folders_names = sorted(glob(os.path.join(inp, "*_y*_HR/")))
    ...
    for tile_folder_name in all_tiles_folders_names:
        ...
        parsed = CHUNK_PATTERN_RUNS.fullmatch(os.path.basename(
            tile_folder_name)) if use_runs else CHUNK_PATTERN.fullmatch(
                os.path.basename(tile_folder_name))

        tile = TileInfo(
            parsed.group("prefix"),
            int(parsed.group("run")),
            int(parsed.group("run") if use_runs else parsed.group("y")),
            1 if use_runs else int(parsed.group("z")),
            parsed.group("suffix"),
            tile_folder_name,
            get_zarr_array(inp, dandiset_id) if using_zarr else SpoolSetInterpreter(
