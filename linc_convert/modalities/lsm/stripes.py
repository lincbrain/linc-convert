...
def create(
    ...
    *,
    ...
    tile_paths_cam1 = discover_tile_paths(
        inp_cam1, dandiset_id=dandiset_id, api_key=api_key)
    tile_paths_cam2 = discover_tile_paths(
        inp_cam2, dandiset_id=dandiset_id, api_key=api_key)

    # --------------------------------------------------
    # Compute affine transforms ONCE using reference tile
    # --------------------------------------------------
    ref_path1 = tile_paths_cam1[ref_tile_idx]
    ref_path2 = tile_paths_cam2[ref_tile_idx]

    name1 = os.path.basename(ref_path1.rstrip("/").replace(".ome.zarr", ""))
    name2 = os.path.basename(ref_path2.rstrip("/").replace(".ome.zarr", ""))

    yx_path1 = os.path.join(
        mip_dir, f"{name1}_proc-mip.tiff").replace("slice0", "slice")
    raw_mip1 = tiff.imread(yx_path1).astype(np.float32)

    yx_path2 = os.path.join(
        mip_dir, f"{name2}_proc-mip.tiff").replace("slice0", "slice")
    raw_mip2 = tiff.imread(yx_path2).astype(np.float32)

    cam1_info = get_camera_info(scanParameters, 1)
    cam2_info = get_camera_info(scanParameters, 2)

    mip_cam1 = crop_mip_channels(raw_mip1, cam1_info)
    mip_cam2 = crop_mip_channels(raw_mip2, cam2_info)

    # Reference = Cam1 594
    ref = mip_cam1["594"]

    def compute_affine(ref, mov):
    ...
    for ch in camera_channel_map[1]:
        tforms[(1, ch)] = compute_affine(ref, mip_cam1[ch])

    for ch in camera_channel_map[2]:
        tforms[(2, ch)] = compute_affine(ref, mip_cam2[ch])

    print("✅ Affine transforms computed once from reference tile")

    chunk = zarr_config.chunk
    if len(chunk) == 1:
        chunk = (chunk[0],) * 3

    for path1, path2 in zip(tile_paths_cam1, tile_paths_cam2):

        start_time = time.time()
        name = os.path.basename(path1.rstrip("/").replace(".ome.zarr", ""))

        reader_cam1 = open_tile_reader(
            path1, dandiset_id=dandiset_id, api_key=api_key)
        reader_cam2 = open_tile_reader(
            path2, dandiset_id=dandiset_id, api_key=api_key)
        ...
        name1 = os.path.basename(
            path1.rstrip("/").replace(".ome.zarr", ""))
        name2 = os.path.basename(
            path2.rstrip("/").replace(".ome.zarr", ""))
        ...
        # Volume extraction
        # -------------------------
        vol_cam1 = crop_volume_channels(reader_cam1, cam1_info)
        vol_cam2 = crop_volume_channels(reader_cam2, cam2_info)

        # -------------------------
        # ✅ Apply affine transforms
        # -------------------------
        def apply_affine(vol, tform):
            tform_inv = np.linalg.inv(tform)
            return affine_transform(vol, matrix=tform_inv, order=3,
                                    mode="nearest", cval=0.0, output_chunks=chunk)

        # Camera 1
        for ch in camera_channel_map[1]:
            vol_cam1[ch] = apply_affine(vol_cam1[ch], tforms[(1, ch)])

        # Camera 2
        for ch in camera_channel_map[2]:
            vol_cam2[ch] = apply_affine(vol_cam2[ch], tforms[(2, ch)])

        # -------------------------
        # Combine all channels
        # -------------------------
        all_vols = {
            **{(1, ch): vol_cam1[ch] for ch in camera_channel_map[1]},
            **{(2, ch): vol_cam2[ch] for ch in camera_channel_map[2]},
        }

        mip_all = {
            **{(1, ch): mip_cam1[ch] for ch in camera_channel_map[1]},
            **{(2, ch): mip_cam2[ch] for ch in camera_channel_map[2]},
        }

        # -------------------------
        # Process each channel
        # -------------------------
        for (cam_id, ch), vol in all_vols.items():

            output_name = f"{general_config.out}/cam{cam_id}_{ch}/{name}.ome.zarr"
            ...
            mask = clean_mask(mask & row_keep[:, None])
            ...
            thr_map = (corr_zy * thr / (zy_max + 1e-6))[:, :, None]
