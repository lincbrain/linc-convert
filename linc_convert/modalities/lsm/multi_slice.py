...
def convert(
    ...
    * `"sagittal"` == `"PI"`
    ...
    for filename in all_chunks_filenames:
        parsed = CHUNK_PATTERN.fullmatch(os.path.splitext(os.path.basename(filename))[0])
    ...
    fullshape[1] = sum(shape[1] for shape in allshapes[0]) - (len(set(all_chunks_info["y"]))-1) * overlap
    ...
    for i, filename in enumerate(all_chunks_info["filename"]):
        ...
        if len(all_chunks_info["y"]) != 1:
            if chunky !=0:
                dat = dat[:,overlap//2:,:]
            elif chunky != len(all_chunks_info["y"]) - 1:
                dat = dat[:,:-overlap//2-(overlap-2*(overlap//2)),:]

        zstart = sum(shape[0][0] for shape in allshapes[:chunkz])
        ystart = sum(
            shape[1]-overlap for shape in allshapes[chunkz][:chunky]
        )
        if chunky != 0:
            ystart += overlap//2
        print(
