...
def stripe_skew_corr_apply(
    ...
    """
    Apply an already-computed (Z, Y) correction map and skew correction
    to a volume, *without* recomputing the correction map.

    This is `stripe_skew_corr`'s steps 2-4 only (threshold, apply the
    correction map, skew-correct). Use this when the correction map has
    been computed once -- e.g. averaged over a handful of sampled tiles
    that are assumed to represent a stable property of the imaging
    system -- and is meant to be reused across many tiles, rather than
    recomputed (a full (Z, Y, X) reduction) for every one of them.

    Like `stripe_skew_corr`, this is correct on any Y-slice of a volume
    independently, since none of these steps mix Y with Z or X.
    ...
    vol : dask.array.Array
        Input volume (Z, Y, X), or a Y-slice of it.
    corr_zy : np.ndarray
        Already-computed (Z, Y) correction map (or a (Z, y_chunk) slice
        of one, matching `vol`'s own Y-range).
    threshold : float
        Intensity threshold (the same value used when `corr_zy` was
        computed).
    camera_id : int
        Camera identifier.
    scan_parameters : dict
