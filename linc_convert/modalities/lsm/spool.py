...
def convert(
    ...
    * `"sagittal"` == `"PI"`
    ...
    TileInfo = namedtuple(
        "TileInfo",
        ["prefix", "run", "y", "z", "suffix", "filename", "array"]
    )
        ...
        all_tiles_info.append(
            TileInfo(
                ...
                read_spool_files(
                    inp,
                    tile_folder_name,
                    os.path.join(inp, f"{tile_folder_name}_info.mat"),
                    no_load=True
                )
            )
        )
    ...
    for z_tile in range(1, num_z_tiles+1):
        for y_tile in range(1, num_y_tiles+1):
            ...
            shapes_x[sz].append((y_tile, z_tile))
            dtypes[dt].append((y_tile, z_tile))

            # Collect Y shapes per z_tile.
            shapes_y_by_z[z_tile][sx].append((y_tile, z_tile))
            # Collect Z shapes per y_tile.
            shapes_z_by_y[y_tile][sy].append((y_tile, z_tile))
    ...
    full_shape_y = sum(tiles_info_by_index[(y_tile, 1)].array.shape[0] for y_tile in
                       range(1, num_y_tiles + 1)) - (num_y_tiles-1) * overlap
    full_shape_z = sum(tiles_info_by_index[(1, z_tile)].array.shape[1] for z_tile in
    ...
    for i, tile_info in enumerate(all_tiles_info):
        ...
        zstart = sum(tiles_info_by_index[(1, z+1)].array.shape[1] for z in range(tile_z))
        ystart = sum(tiles_info_by_index[(y+1, tile_info.z)].array.shape[0] - overlap for y in range(tile_y))
        # zstart = sum(shape[0][0] for shape in allshapes[:tile_z])
        # ystart = sum(shape[1] - overlap for shape in allshapes[tile_z][:tile_y])
        if tile_y != 0:
            ystart += overlap // 2
        print(
            f"Write plane "
...
def read_spool_files(scan_path: str, scan_name: str, mat_path: str, no_load: bool = False) -> np.ndarray:
    """
