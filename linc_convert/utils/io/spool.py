...
class SpoolSetInterpreter:
    ...
    This class loads spool files from a directory, parses acquisition parameters,
    and provides methods to iterate over or assemble the image data.
    """
    ...
    def _what_spool_format(self) -> None:
    ...
    def _list_spool_files(self) -> list[str]:
        return sorted(tuple([x for x in self.entries if "0spool.dat" in x]))

    def _get_acquisitionparameters_str(self) -> None:
    ...
    def _extract_config_values(self) -> None:
        ...
        self.acquisition_metadata["nbytes"]
    ...
    def __getitem__(self, key: str | int) -> np.ndarray:
        ...
        if isinstance(key, str):
            assert key in self.spool_files, (
                "Must be a spool file in self.spool_files OR integer index in "
                "self.spool_files"
            )
    ...
    def _get_spool_names_in_order(self) -> Iterator[str]:
        """
        Get list of sorted spool names.

        Spool files are ordered sequentially in the order they were collected 0,1,2,
        ...,201,202,203,...
        but file names are recorded as the reversed number padded to 10 digits
        (0000000000,1000000000,20000000000,...,1020000000,2020000000,3020000000,
        ...) + spool.dat.
        """
